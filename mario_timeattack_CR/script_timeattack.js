...
const TARGET_SCORE = 1000; // タイムアタックの目標スコア
...
// 新しいゲームモード関連の変数 (timeAttackモード用)
let gameMode = 'timeAttack'; // このファイルはtimeAttackモード
let startTime = 0;
let elapsedTime = 0; // 経過時間用
...
function drawMessage(message, subMessage, finalScore) {
    ...
    if (gameMode === 'timeAttack' && gameState === 'cleared') {
        ctx.font = '24px sans-serif';
        ctx.fillText(`クリアタイム: ${elapsedTime}秒`, canvas.width / 2, canvas.height / 2 + 20);
    }
    ctx.font = '24px sans-serif';
...
function drawScore() {
    ...
    if (gameMode === 'timeAttack') {
        ctx.fillText(`経過時間: ${elapsedTime}秒`, 20, 70);
    }
}
...
function animate() {
    ...
    if (gameState === 'playing') {
        ...
        // ゴール判定 (タイムアタックモードではスコアでクリア)
        if (gameMode === 'timeAttack' && score >= TARGET_SCORE) {
            gameState = 'cleared';
            clearInterval(timerInterval); // タイマーを停止
        } else if (player.position.x > goal.position.x) {
            gameState = 'cleared';
        }
