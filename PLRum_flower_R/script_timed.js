...
const MODE_TARGET_TIME = 60; // 各モードの目標滞在時間（秒）（このバージョンでは使用しない）
...
// 左右切替関連の変数（常に右利きモード）
let isRightHanded = true; // 常に右利き（左スティック+Bボタン）
...
// 左右切替のスケジュール関数は削除（常に右利きモードのため不要）
...
function drawMessage(message, subMessage, finalScore) {
    ...
    if (gameMode === 'timed' && gameState === 'gameOver' && finalScore !== undefined) {
        ...
        if (plumBloomTriggered && flowerImage && flowerImage.complete) {
            const rotationSpeed = 1.0; // 回転速度（クルクル回る速度）
...
window.addEventListener('keyup', ({ code }) => { 
    ...
    switch (code) { 
        case 'ArrowLeft': case 'KeyA': keys.left.pressed = false; break; 
        case 'ArrowRight': case 'KeyD': keys.right.pressed = false; break; 
    } 
});
