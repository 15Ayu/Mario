...
let rightHandedTime = 0; // 右利きモードの累計時間（秒）
let leftHandedTime = 0; // 左利きモードの累計時間（秒）
let currentModeStartTime = 0; // 現在のモードが開始した時刻
...
function scheduleNextSwitch() {
    // 各モードが60秒になるように制御
    const targetTime = 60; // 目標時間（秒）
    const now = Date.now();
    
    // 現在のモードの累計時間を計算（既存の累計時間 + 現在のモードの経過時間）
    let currentTime = isRightHanded ? rightHandedTime : leftHandedTime;
    if (currentModeStartTime > 0) {
        const elapsedTime = (now - currentModeStartTime) / 1000; // 秒単位
        currentTime += elapsedTime;
    }
    
    const remainingTime = targetTime - currentTime;
    
    // 残り時間が0以下、または既に60秒に達している場合は即座に切り替え
    if (remainingTime <= 0) {
        nextSwitchTime = now;
        console.log(`モード累計時間が目標に達しました。即座に切り替えます。（累計: ${currentTime.toFixed(1)}秒 / 目標: ${targetTime}秒）`);
    } else {
        // 残り時間を切り替え時刻に設定（最大でも残り時間分、最小でも1秒）
        const switchDelay = Math.max(1000, Math.min(remainingTime * 1000, 30000)); // 1秒〜30秒
        nextSwitchTime = now + switchDelay;
        console.log(`次の切り替え予定: ${Math.round(switchDelay / 1000)}秒後（累計: ${currentTime.toFixed(1)}秒 / 目標: ${targetTime}秒）`);
    }
}
...
function init() {
    ...
    rightHandedTime = 0; // 右利きモードの累計時間をリセット
    leftHandedTime = 0; // 左利きモードの累計時間をリセット
    currentModeStartTime = 0; // 現在のモード開始時刻をリセット
    scheduleNextSwitch();
        ...
        timerInterval = setInterval(() => {
            if (gameState === 'playing') {
                ...
                if (remainingTime <= 0) {
                    ...
                    // ゲーム終了時に現在のモードの累計時間を最終更新
                    const now = Date.now();
                    if (currentModeStartTime > 0) {
                        const finalElapsedTime = (now - currentModeStartTime) / 1000;
                        if (isRightHanded) {
                            rightHandedTime += finalElapsedTime;
                        } else {
                            leftHandedTime += finalElapsedTime;
                        }
                    }
                    console.log(`ゲーム終了 - 右利き: ${rightHandedTime.toFixed(1)}秒 / 左利き: ${leftHandedTime.toFixed(1)}秒`);
...
function drawMessage(message, subMessage, finalScore) {
    ...
    if (gameMode === 'timed' && gameState === 'gameOver' && finalScore !== undefined) {
        ...
        // 各モードの累計時間を表示
        const timeLabelY = statsY + spacing * 3;
        const rightTimeText = `右利きモード: ${rightHandedTime.toFixed(1)}秒 / 60秒`;
        const leftTimeText = `左利きモード: ${leftHandedTime.toFixed(1)}秒 / 60秒`;
        
        // 右利きモードの時間を表示
        ctx.fillStyle = '#FF0000'; // 赤色
        ctx.strokeText(rightTimeText, centerX, timeLabelY);
        ctx.fillText(rightTimeText, centerX, timeLabelY);
        
        // 左利きモードの時間を表示
        ctx.fillStyle = '#0066FF'; // 青色
        ctx.strokeText(leftTimeText, centerX, timeLabelY + spacing);
        ctx.fillText(leftTimeText, centerX, timeLabelY + spacing);
...
function drawScore() {
    ...
    if (gameState === 'playing' && timerStarted) {
        ...
        // 現在のモードの経過時間も計算
        const now = Date.now();
        let currentRightTime = rightHandedTime;
        let currentLeftTime = leftHandedTime;
        
        if (currentModeStartTime > 0) {
            const currentElapsed = (now - currentModeStartTime) / 1000;
            if (isRightHanded) {
                currentRightTime += currentElapsed;
            } else {
                currentLeftTime += currentElapsed;
            }
        }
        
        const timeTextY = canvas.height - 100;
        const rightTimeText = `右利き: ${currentRightTime.toFixed(1)}秒 / 60秒`;
        const leftTimeText = `左利き: ${currentLeftTime.toFixed(1)}秒 / 60秒`;
        
        // 右利きモードの時間を表示
        ctx.fillStyle = '#FF0000'; // 赤色
        ctx.strokeText(rightTimeText, 15, timeTextY);
        ctx.fillText(rightTimeText, 15, timeTextY);
        
        // 左利きモードの時間を表示
        ctx.fillStyle = '#0066FF'; // 青色
        ctx.strokeText(leftTimeText, 15, timeTextY + 30);
        ctx.fillText(leftTimeText, 15, timeTextY + 30);
    ...
    if (gameState === 'playing' && timerStarted) {
        ...
        if (now >= nextSwitchTime) {
            ...
            if (currentModeStartTime > 0) {
                const elapsedTime = (now - currentModeStartTime) / 1000; // 秒単位
                if (isRightHanded) {
                    rightHandedTime += elapsedTime;
                } else {
                    leftHandedTime += elapsedTime;
                }
            }
            
            isRightHanded = !isRightHanded;
            currentModeStartTime = now; // 新しいモードの開始時刻を記録
