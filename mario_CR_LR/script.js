...
function drawScore() { 
    ...
    if (gamepadConnected) {
        ...
        ctx.fillText('コントローラー接続中', 20, 70);
    } else {
        ctx.fillStyle = 'red';
        ctx.font = '16px sans-serif';
        ctx.fillText('コントローラー未接続', 20, 70);
    }
}
...
function animate() {
    ...
    if (connectedGamepad) {
        ...
        if (rightStickX < -0.5) { // 左に倒す
            ...
            keys.right.pressed = false;
            console.log("左移動 - keys.left.pressed:", keys.left.pressed);
        } else if (rightStickX > 0.5) { // 右に倒す
            keys.right.pressed = true;
            keys.left.pressed = false;
            console.log("右移動 - keys.right.pressed:", keys.right.pressed);
        } else { // ニュートラル
            keys.left.pressed = false;
            keys.right.pressed = false;
        }
        ...
        if (gamepad.buttons[15] && gamepad.buttons[15].pressed) {
            if (!gamepadButtons.rightPressed && gameState === 'playing') {
                ...
                if (player.velocity.y === 0 || player.velocity.y > -2) {
                    ...
                    console.log("ジャンプ実行 - velocity.y:", player.velocity.y);
                }
            }
        } else {
    ...
    if (gameState === 'playing') {
        ...
        if (keys.right.pressed) {
            ...
            console.log("右移動実行 - velocity.x:", player.velocity.x);
        } else if (keys.left.pressed) {
            player.velocity.x = -PLAYER_SPEED;
            console.log("左移動実行 - velocity.x:", player.velocity.x);
        } else {
            player.velocity.x = 0;
        }
        ...
        if (keys.right.pressed || keys.left.pressed) {
            console.log("プレイヤー位置 - x:", player.position.x, "y:", player.position.y);
        }
    ...
    if (gameState === 'cleared') drawMessage('クリア！', 'Enterキーまたは十字ボタン左でリスタート', score);
    if (gameState === 'gameOver') drawMessage('ゲームオーバー', 'Enterキーまたは十字ボタン左でリスタート', score);
}
